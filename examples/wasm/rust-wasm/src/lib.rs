...
use proxy_wasm::{host, HeaderMapType};
...
fn proxy_on_response_headers(p0: i32, p1: i32) -> i32 {
    log::debug!(
        "rust: `proxy_on_response_headers` called, params: p0={} p1={}",
        p0,
        p1
    );
    0
...
#[no_mangle]
fn proxy_on_request_headers(p0: u32, p1: u32) -> u32 {
    unsafe {
        let data: *const u8 = "".as_ptr();
        host::proxy_get_header_map_pairs(HeaderMapType::RequestHeaders, data, 1024);
        log::debug!("rust headers: {:?} !!!!!!!!", *data);
    }
    log::debug!(
        "rust: `proxy_on_request_headers` called, params: p0={} p1={}",
        p0,
        p1
    );
    0
